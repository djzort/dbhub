...
int perl_init(void)
{
   ...
   for(i = 0; i <= k; i++)
	...
	if(pid > 0)
	  {
	     logprintf(LOG_EMERG, "Forked new script parsing process for script %s, childs pid is %d and parents pid is %d\n", script_list[i], pid, getpid());
	     pid = getpid();
	  }
	...
	/* And if we are the child */
	else
	  {
	     ...
	     while(((erret =  close(listening_unx_socket)) != 0) && (errno == EINTR))
	       logprintf(LOG_EMERG, "Error - In perl_init()/close(): Interrupted system call. Trying again.\n");
	     
	     if(erret != 0)
	       {		  
		  logprintf(LOG_EMERG, "Error - In perl_init()/close(): %s",strerror(errno));
	       }
	     ...
	     if((sock = socket(AF_UNIX, SOCK_STREAM, 0)) == -1)
	       {		  
		  logprintf(LOG_EMERG, "Error - In perl_init()/socket(): %s",strerror(errno));
		  free(script_list[i]);
		  exit(EXIT_FAILURE);
	       }
	     ...
	     if(connect(sock, (struct sockaddr *)&remote_addr, len) == -1)
	       {	     
		  logprintf(LOG_EMERG, "Error - In perl_init()/connect(): %s",strerror(errno));
		  free(script_list[i]);
		  exit(EXIT_FAILURE);
	       }
	     ...
	     if((non_human_user_list = malloc(sizeof(struct user_t))) == NULL)
	       {		  
		  logprintf(LOG_EMERG, "Error - In parl_init()/malloc(): %s",strerror(errno));
		  quit = 1;
		  free(script_list[i]);
		  exit(EXIT_FAILURE);
	       }	     	     
	     ...
	     non_human_user_list->myinfos = NULL;
	     non_human_user_list->myinfons = NULL;
	     non_human_user_list->myinfoons = NULL;
	     non_human_user_list->con_type = NULL;
	     ...
	     if((my_perl = perl_alloc()) == NULL) 
	       {	
		  logprintf(LOG_EMERG, "perl_alloc() failed\n");
		  free(script_list[i]);
		  exit(EXIT_FAILURE);
	       }
	     ...
	     if(perl_parse(my_perl, xs_init, 2, myargv, NULL))
	       {
		  logprintf(LOG_EMERG, "Parse of %s failed.\n", script_list[i]);
		  free(script_list[i]);
		  exit(EXIT_FAILURE);
	       }
	     ...
	     if(perl_run(my_perl))
	       {
		  logprintf(LOG_EMERG, "Couldn't run perl script %s.\n", script_list[i]);
		  free(script_list[i]);
		  exit(EXIT_FAILURE);
	       }
	     ...
	     if(i == 0)
		  ...
		  if((buf = (char *)shmat(get_user_list_shm_id(), NULL, 0))
		     == (char *)-1)
		    {		       
		       logprintf(LOG_EMERG, "Error - In perl_init()/shmat(): %s",strerror(errno));
		  ...
		  if(sscanf(buf, "%d %d", &spaces, &entries) != 2)
		    {		       
		       logprintf(LOG_EMERG, "Error - In perl_init(): Couldn't get number of entries");
		       shmdt(buf);
		       sem_give(user_list_sem);
		       quit = 1;
		       return -1;
		  ...
		  for(l = 1; l <= spaces; l++)
		    {		       
			 ...
			 {			    
			    ...
			    uprintf(non_human_user_list, "$ScriptINFO %s $Script|", temp_nick);
			 }		       
...
void sub_to_script(char *buf)
{
   ...
   if(sscanf(buf, "%30[^| ]", subname) != 1)
     {
	logprintf(LOG_EMERG, "Got incomplete command to to_script()\n");
	return;
     }
   ...
   if(!strncmp(subname, "user_info", 9))
	...
	if((temp_user = get_human_user(temp_nick)) == NULL)
	  {	     
	    if((temp_user = malloc(sizeof(struct user_t))) == NULL)
	       {		  
		  logprintf(LOG_EMERG, "Error - In sub_to_script()/malloc(): %s",strerror(errno));
		  quit = 1;
		  return;
	       }
	     
	     temp_user->email = NULL;
	     temp_user->con_type = NULL;
	     temp_user->desc = NULL;
	     temp_user->buf = NULL;
	     temp_user->outbuf = NULL;
	     temp_user->myinfos = NULL;
	     temp_user->myinfons = NULL;
	     temp_user->myinfoons = NULL;
	  }
	else
	  {
	     ...
	     if(temp_user->con_type != NULL)
	       free(temp_user->con_type);
	     temp_user->con_type = NULL;
	     ...
	     if(temp_user->myinfos != NULL)
	       free(temp_user->myinfos);
	     temp_user->myinfos = NULL;

	     if(temp_user->myinfons != NULL)
	       free(temp_user->myinfons);
	     temp_user->myinfons = NULL;

	     if(temp_user->myinfoons != NULL)
	       free(temp_user->myinfoons);
	     temp_user->myinfoons = NULL;
	...
	sscanf(buf + 10, "%50s %120s %llu|", temp_user->nick, 
	       temp_user->hostname, &temp_user->share);	

	return;
     }

   if(!strncmp(subname, "stat_info", 9))
     {
	sscanf(buf + 10, "%d %llu|", &total_users_scr, &total_share_scr);	
        return;
     }
